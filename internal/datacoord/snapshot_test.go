...
func TestValidateSnapshotObjectPath(t *testing.T) {
	...
	tests := []struct {
		...
		},
		{
			...
			path:    "https://test-bucket/files/snapshots/100/metadata/1.json",
			wantErr: "snapshot_s3_location must be an object key or s3 URI",
		},
		{
			name:    "missing bucket",
			path:    "s3:///files/snapshots/100/metadata/1.json",
			wantErr: "snapshot_s3_location s3 URI must include a bucket",
		},
		{
