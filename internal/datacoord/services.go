...
func (s *Server) CreateSnapshot(ctx context.Context, req *datapb.CreateSnapshotRequest) (*commonpb.Status, error) {
	...
	log.Info("generated snapshot data", zap.Any("snapshotData", snapshotData.SnapshotInfo))
...
func (s *Server) RestoreSnapshot(ctx context.Context, req *datapb.RestoreSnapshotRequest) (*datapb.RestoreSnapshotResponse, error) {
	...
	if len(snapshotData.Segments) == 0 {
		log.Info("snapshot has no segments, nothing to restore")
		return &datapb.RestoreSnapshotResponse{
			Status: merr.Status(merr.WrapErrParameterInvalidMsg("no segments to restore")),
		}, nil
	}
...
// restoreSnapshotByCopy creates a single copy segment job for all segments
func (s *Server) restoreSnapshotByCopy(
	...
	if len(snapshotInfo.Segments) == 0 {
		return 0, merr.WrapErrImportFailed("no segments to restore")
	}
	...
	if len(validSegments) == 0 {
		return 0, merr.WrapErrImportFailed("no valid segments found in meta")
	}
	...
	for i, segDesc := range validSegments {
		...
		newSegment := &SegmentInfo{
			SegmentInfo: &datapb.SegmentInfo{
				...
				StartPosition:       segDesc.GetStartPosition(),
				DmlPosition:         segDesc.GetDmlPosition(),
