...
func (sm *snapshotManager) ExportSnapshot(
	...
	log.Info("export snapshot request received")
	...
	if err != nil {
		log.Warn("failed to read snapshot data for export", zap.Error(err))
		return "", err
	}
	...
	if err != nil {
		log.Warn("failed to export snapshot", zap.Error(err))
		return "", err
	}
	log.Info("export snapshot completed", zapSnapshotMetadataURI(metadataURI))
