...
func validateSnapshotObjectPathShape(fieldName string, objectPath string) error {
	...
	if parsed.Scheme != "s3" {
		return fmt.Errorf("%s must be an object key or s3 URI", fieldName)
	}
	if parsed.Host == "" {
		return fmt.Errorf("%s s3 URI must include a bucket", fieldName)
	}
	return nil
}
...
func validateSnapshotObjectPathForBucket(cm storage.ChunkManager, fieldName string, objectPath string, expectedBucket string) error {
	...
	if bucket != "" && parsed.Host != bucket {
		return fmt.Errorf("%s bucket %q does not match configured bucket %q", fieldName, parsed.Host, bucket)
	}
	return nil
}
