...
func NewSegmentTask(ctx context.Context,
	...
	if len(actions) == 0 {
		return nil, errors.New("empty actions is not allowed")
	}
	...
	for _, action := range actions {
		...
		if !ok {
			return nil, errors.New("SegmentTask can only contain SegmentActions")
		}
		if segmentID == -1 {
			segmentID = action.SegmentID()
			shard = action.Shard()
		} else if segmentID != action.SegmentID() {
			return nil, errors.New("all actions must process the same segment")
		}
	}
...
func NewChannelTask(ctx context.Context,
	...
	for _, action := range actions {
		...
		if !ok {
			return nil, errors.New("ChannelTask must contain only ChannelAction")
		}
		if channel == "" {
			channel = channelAction.ChannelName()
		} else if channel != channelAction.ChannelName() {
			return nil, errors.New("all actions must process the same channel")
		}
	}
