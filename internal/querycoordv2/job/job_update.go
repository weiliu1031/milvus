...
func (job *UpdateLoadConfigJob) Execute() error {
	...
	for rg, replicas := range toTransfer {
		for _, replica := range replicas {
			...
			if transferErr := job.meta.ReplicaManager.TransferReplica(replica.GetID(), replica.GetResourceGroup(), rg, 1); transferErr != nil {
				log.Warn("failed to transfer replica", zap.Int64("replica", replica.GetID()), zap.Error(transferErr))
				err = transferErr
				return err
			}
		}
	...
	// 7. update next target, no need to rollback if pull target failed, target observer will pull target in periodically
	...
	// 8. register load task into collection observer
	// todo: think about load percent related issues
	// 1. replica changes shouldn't reduce load percent
	// 2. ....
