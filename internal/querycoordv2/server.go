...
func (s *Server) watchLoadConfigChanges() {
	handler := config.NewHandler("watchLoadConfigChanges", func(e *config.Event) {
		...
		var err error
		rgs := make([]string, 0)
		if e.Key == paramtable.Get().QueryCoordCfg.ClusterLevelLoadReplicaNumber.Key {
		...
		if e.Key == paramtable.Get().QueryCoordCfg.ClusterLevelLoadResourceGroups.Key {
	...
	paramtable.Get().Watch(paramtable.Get().QueryCoordCfg.ClusterLevelLoadReplicaNumber.Key, handler)
	paramtable.Get().Watch(paramtable.Get().QueryCoordCfg.ClusterLevelLoadResourceGroups.Key, handler)
}
