...
func (iNode *insertNode) Operate(in []flowgraph.Msg) []flowgraph.Msg {
	...
	for _, insertMsg := range iMsg.insertMessages {
		...
		if !has {
			err = iNode.metaReplica.addSegment(insertMsg.SegmentID, insertMsg.PartitionID, insertMsg.CollectionID, insertMsg.ShardName, 0, insertMsg.MsgPosition, segmentTypeGrowing)
			if err != nil {
