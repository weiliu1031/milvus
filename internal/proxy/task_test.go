...
func TestDescribeCollectionTask_RedactsExternalSpecCredentials(t *testing.T) {
	...
	assert.NotContains(t, out, "AKIAEXAMPLE", "access_key_id must not leak")
	assert.NotContains(t, out, "SUPERSECRET", "access_key_value must not leak")
	assert.Contains(t, out, `"access_key_id":"***"`)
	assert.Contains(t, out, `"access_key_value":"***"`)
	// Non-secret extfs values must still be visible.
